...
{ unsigned int i;
	...
	// TODO: Becase the exact pitch is not known at init time:
	si->rowbuf    = (uchar*)malloc( size_out.w * fmt_out.step * 2 );
	for( i = 0; i < size_out.w * fmt_out.step * 2; i++ )
	{	si->rowbuf[i] = 0;  }
	init_cells( si );
	init_bound_cols( si );
	return res;
...
{	info* si = (info*)osi;
	...
	scale_init( si, &pix_in, pix_out, &mid_row, area, &width_bytes, &rs );
	...
	{	src  = rs.in;
		...
		if( nb )
		{	loopx_nb( si, area, src, mid_row, &rs );  }
		else
		{	loopx_np( si, area, src, mid_row, &rs, ri, pix_in, pix_out );  }
		...
		{	mid_row_out = rs.out +
			...
			{	memcpy(	mid_row_out, mid_row, width_bytes );
